"""
repo_cloner.py — Handles cloning of GitHub repositories.
"""
...
def clone_repository(repo_url: str, dest_dir: str) -> str:
    ...
    If the repository is already cloned, fetch the latest changes.

    Args:
        repo_url: GitHub HTTPS URL (e.g. https://github.com/owner/repo).
    ...
    if os.path.exists(repo_path):
        logger.info(f"Repository already exists at {repo_path}. Fetching latest changes...")
        try:
            ...
            logger.warning(f"Fetch failed ({e}). Removing stale clone and re-cloning...")
...
def _do_clone(repo_url: str, repo_path: str) -> None:
    ...
    try:
        git.Repo.clone_from(repo_url, repo_path, depth=None)
...
def _extract_repo_name(repo_url: str) -> str:
    """Extract a filesystem-safe name from the repo URL."""
